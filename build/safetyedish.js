(function(global, factory) {
    ...
    function addExposure() {
        ...
        if (this.edish.exposure.include) {
            var exposures = this.edish.exposure.data.filter(function(d) {
                return d[_this.edish.config.id_col] === _this.edish.clicked_id;
            });
            var g = this.svg.insert('g', '.supergroup').classed('se-exposure-group', true);
            var rects = g
                ...
                .attr({
                    x: function x(d) {
                        return _this.x(+d.EXSTDY);
                    },
                    y: 0,
                    width: function width(d) {
                        return d.EXSTDY !== d.EXENDY ? _this.x(+d.EXENDY) - _this.x(+d.EXSTDY) : 1; // for single day exposures
                    },
            ...
            var titles = rects.append('title').text(function(d) {
                return (
                    ...
                    d.EXENDY +
                    ' (' +
                    (+d.EXENDY - +d.EXSTDY + (+d.EXENDY >= +d.EXSTDY)) +
                    ' days)\nExposure: ' +
                    d.EXTRT
                );
            });
