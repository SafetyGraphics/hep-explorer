(function (global, factory) {
    ...
    function setPointSize() {
        ...
        if (config.point_size !== 'Uniform') {
            ...
            var sizeValues_all = d3.merge(this.raw_data.map(function (d) {
            ...
            var sizeDomain = config.point_size === 'rRatio' ? [0, d3.max(this.raw_data, function (d) {
                return d.rRatio_relative_uln;
            })] : config.point_size === 'nrRatio' ? [0, d3.max(this.raw_data, function (d) {
                return d.nrRatio_relative_uln;
            })] : config.plot_max_values ? d3.extent(this.raw_data.map(function (m) {
            ...
            this.sizeScale = d3.scale.linear().domain(sizeDomain).range([base_size, max_size]);
        }

        //TODO: draw a legend (coming later?)

        // Set the point radius.
        this.marks[0].circles.transition().attr('r', function (d) {
            var raw = d.values.raw[0];
            if (raw.outOfRange) {
                return small_size;
            } else if (config.point_size == 'Uniform') {
