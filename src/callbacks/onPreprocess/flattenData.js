export function flattenData(){
  ...
  var included_measures = this.config.measure_details.map(m=>m.measure)
  ...
  var flat_data = d3.nest()
  .key(this.config.id_col)
  .rollup(function(d){
    var participant_obj = {}
    this.config.measure_details.forEach(m){
      var matches = d.filter(f=> m.measure == f[this.config.measure_col]) //get matching measures

      //get max raw value
      participant_obj[m.label + "_absolute"] = d3.max(matches, d=>d[config.value_col])
      participant_obj[m.label +"_absolute_flagged"] = participant_obj[m.label + "_absolute"] > m.cut.absolute

       //get max relative value and flagged status
      participant_obj[m.label + "_relative"] = d3.max(matches, d=>d.relative)
      participant_obj[m.label +"_relative_flagged"] = participant_obj[m.label + "_relative"] > m.cut.relative
    }
