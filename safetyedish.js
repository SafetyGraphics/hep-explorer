(function (global, factory) {
    ...
    function formatRRatioControl() {
        ...
        if (this.config.r_ratio_filter) {
            ...
            //add a reset button
            min_r_ratio.append('button').style('padding', '0.2em 0.4em 0.2em 0.4em').style('margin-left', '0.5em').text('Reset').on('click', function () {
                config.r_ratio[0] = 0;
    ...
    function initControlLabels() {
        ...
        this.controls.setting_header = first_setting.insert('div', '*').attr('class', 'subtitle').style('border-top', '1px solid black').style('border-bottom', '1px solid black').style('margin-right', '1em').style('margin-bottom', '1em');
        ...
        if (config.r_ratio_filter || config.filters.length > 0) {
            ...
            var control_wraps = this.controls.wrap.selectAll('div').filter(function (controlInput) {
                return controlInput.label === 'Minimum R Ratio' || controlInput.type === 'subsetter';
            }).classed('subsetter', true);

            var first_filter = this.controls.wrap.select('div.subsetter');

            this.controls.filter_header = first_filter.insert('div', '*').attr('class', 'subtitle').style('border-top', '1px solid black').style('border-bottom', '1px solid black').style('margin-right', '1em').style('margin-bottom', '1em');
    ...
    function addFootnote() {
        this.wrap.append('div').attr('class', 'footnote').text('Use controls to update chart or click a point to see participant details.').style('font-size', '0.7em').style('padding-top', '0.1em');
    }
    ...
    function setMaxRRatio() {
        ...
        if (this.config.r_ratio_filter) {
            if (!config.r_ratio[1]) {
                ...
                console.log(raw_max_r_ratio + '->' + config.max_r_ratio);
                config.r_ratio[1] = config.max_r_ratio;
                r_ratio_wrap.select('input#r_ratio_max').property('value', config.max_r_ratio);
            }
    ...
    function flattenData() {
        ...
        var colList = [];
        var measureCols = ['measure_col', 'value_col', 'studyday_col', 'normal_col_low', 'normal_col_high'];
        ...
        colList = d3.merge([colList, ['absolute', 'relative_uln', 'relative_baseline', 'baseline_absolute', 'analysisFlag']]);
